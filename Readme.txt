...
이 코드를 사용하기위해선 이 .cpp파일을 파이선모듈화 해야한다. 그냥 쓰고 왜안되지? 이러지말고 반드시 이 코드를 파이선 모듈화하는 방법을 찾아봐서
사용하길 바란다.
...
TX1보드가 만일 초기화 되어있지 않다면 이 모든 내용들은 ~/capston/thread폴더 안에있을겁니다.
저 폴더내에서 2,3번 명령어를 차례대로 실행시키면 올바르게 실행은 될겁니다.
그리고 알아서 필요한 기능 수정 및 추가해서 쓰시면됩니다.
